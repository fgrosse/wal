...
func TestSegmentWriter_Write_Size(t *testing.T) {
	...
	err := sw.Write(42, ExampleEntryType, uint32(0x470b99f4), []byte{1, 2, 3, 4, 5})
	require.NoError(t, err)
	assert.Equal(t, 4+1+4+5, sw.size) // Offset + Type + CRC + Payload

	err = sw.Write(43, ExampleEntryType, uint32(0x470b99f4), []byte{'a', 'b', 'c'})
	require.NoError(t, err)
...
func TestNewSegmentWriter_Close(t *testing.T) {
	...
	checksum := uint32(0x470b99f4)
	err := sw.Write(1, ExampleEntryType, checksum, entry)
	require.NoError(t, err)
